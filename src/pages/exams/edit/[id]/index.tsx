...
import { useRoqClient, useExamFindFirst } from 'lib/roq';
...
function ExamEditPage() {
  ...
  const { data, error, isLoading, mutate } = useExamFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ExamInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.exam.update({
        data: values as RoqTypes.exam,
        where: {
          id,
        },
      });
