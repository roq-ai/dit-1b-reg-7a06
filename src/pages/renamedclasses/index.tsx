...
import { useRoqClient, useRenamedclassFindManyWithCount } from 'lib/roq';
...
export function RenamedclassListPage(props: RenamedclassListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['school', 'user', 'attendance.count', 'student.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['name.contains', 'class_subject.contains'],
          ...params.filters,
        },
        'renamedclass',
      ),
  ...
  const { data, error, isLoading, mutate } = useRenamedclassFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.renamedclass.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'class_subject', header: 'Class Subject', accessorKey: 'class_subject' },
